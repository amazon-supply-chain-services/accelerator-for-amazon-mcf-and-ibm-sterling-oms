...
public class AmzGetAmazonRefundOrdDetail {
	
	private static final YFCLogCategory logger = YFCLogCategory.instance(AmzGetAmazonRefundOrdDetail.class);
	
	public static Document getOrderDetails(YFSEnvironment env, Document doc) throws Exception {
		logger.beginTimer("class: AmzGetAmazonRefundOrdDetail | method: getOrderDetails -- Starts");
		...
		try {
	        ...
	        String query = AmzOrderMutations.AMZ_GET_REFUND_ORDER_DETAILS;
	        String enterpriseCode = eleDoc.getAttribute(AmzLiterals.ATTR_ENTERPRISE_CODE);
	        logger.debug("class: AmzGetAmazonRefundOrdDetail | method: getOrderDetails | enterpriseCode : "+enterpriseCode);
		...
		logger.endTimer("class: AmzGetAmazonRefundOrdDetail | method: getOrderDetails -- Ends");
		return outDoc;
	}
	
	public static void prepareAndLogResponse(String processStatus, Document inDoc, String message) {

		logger.beginTimer("class: AmzGetAmazonRefundOrdDetail | method: prepareAndLogResponse -- Starts");
		...
		logger.endTimer("class: AmzGetAmazonRefundOrdDetail | method: prepareAndLogResponse -- Ends");
	}
}
