 ...
 * for SHIPMENT_STATUS_CHANGED events only, and posts the event XML as-is
 * to the internal queue for processing by AmzProcessMCFShipmentEvent.
 *
 * Input Sample - SHIPMENT_STATUS_CHANGED (Shipped):
 ...
 */
public class AmzReceiveMCFShipmentStatus implements YIFCustomApi {
	private static final YFCLogCategory logger = YFCLogCategory.instance(AmzReceiveMCFShipmentStatus.class);
	private static final String EXPECTED_EVENT_TYPE = "SHIPMENT_STATUS_CHANGED";
	...
	public Document processEventAndPostToQueue(YFSEnvironment env, Document inputDoc)
		...
		logger.beginTimer("class: AmzReceiveMCFShipmentStatus | method: processEventAndPostToQueue -- Starts");
		logger.info("class: AmzReceiveMCFShipmentStatus | method: processEventAndPostToQueue -- Starts");
		logger.info("class: AmzReceiveMCFShipmentStatus | method: processEventAndPostToQueue Input Doc is:"
		...
		try {
			...
			if ("Y".equals(errorMap.get(AmzLiterals.STR_IS_VLD_MSG))) {
				...
				logger.error("Exception in AmzReceiveMCFShipmentStatus.processEventAndPostToQueue Method: "
		...
		logger.endTimer("class: AmzReceiveMCFShipmentStatus | method: processEventAndPostToQueue -- Ends");
		logger.info("class: AmzReceiveMCFShipmentStatus | method: processEventAndPostToQueue -- Ends");
		return responseDoc;
	}

	private void prepareAndErrorLogResponse(Document inputDoc, String errorCode, String errorDescription) {
		logger.beginTimer("class: AmzReceiveMCFShipmentStatus | method: prepareAndErrorLogResponse -- Starts");
		...
		logger.endTimer("class: AmzReceiveMCFShipmentStatus | method: prepareAndErrorLogResponse -- Ends");
	}

	private HashMap<String, String> validateInputMsg(Document inDoc) {
		logger.beginTimer("class: AmzReceiveMCFShipmentStatus | method: validateInputMsg -- Starts");
		...
		try {
			...
			if (YFCCommon.isVoid(eventType)) {
				...
				strErrorMsg = "Unsupported eventType: " + eventType
						+ ". Only SHIPMENT_STATUS_CHANGED is accepted.";
			...
			logger.endTimer("class: AmzReceiveMCFShipmentStatus | method: validateInputMsg -- Ends");
		} catch (Exception ex) {
			YFSException yfsException = new YFSException();
			yfsException.setErrorCode("INPUT_ERR_WHILE_VALIDATING");
			yfsException.setErrorDescription("Error While Validating the Input");
			logger.error("Exception in AmzReceiveMCFShipmentStatus.validateInputMsg Method: "
					+ ExceptionUtils.getStackTrace(yfsException));
