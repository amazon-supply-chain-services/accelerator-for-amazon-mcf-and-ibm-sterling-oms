...
/**
 * A utility class to generate token for SP API invocation. The class exposes
 * getToken API as public method. The client is expected to pass the required
 * credentials to connect to the token system.
 * 
 */

public class SPApiTokenGenerationUtil {

	private static final YFCLogCategory logger = YFCLogCategory.instance(SPApiTokenGenerationUtil.class);

	private static SPApiToken tokenObj = null;
	 ...
	 */
	public static SPApiToken getToken(String enterpriseCode) throws Exception {
		logger.beginTimer("class: SPApiTokenGenerationUtil | method: getToken -- Starts");
		if (isDebugEnabled) {
		 ...
		 *
		 **/
		if (tokenObj == null || !tokenObj.spApiTokenMap.containsKey("sp_token_" + enterpriseCode)) {
			...
			logger.debug("Obtained a new Token. Returning token value: " + tokenObj.spApiTokenMap.get("sp_token_" + enterpriseCode));
	 ...
	 */
	private static boolean isTokenValid(SPApiToken tokenObj, String enterpriseCode) throws Exception {
		logger.beginTimer("class: SPApiTokenGenerationUtil | method: isTokenValid -- Starts");
		...
		logger.debug("Token generate time in seconds is: " + tokenObj.spApiTokenMap.get("sp_tokenGeneratedTime_" + enterpriseCode));
		logger.debug("Token Validity in seconds is: " + tokenObj.spApiTokenMap.get("sp_validity_" + enterpriseCode));

		int tokenValidity = Integer.parseInt(tokenObj.spApiTokenMap.get("sp_validity_" + enterpriseCode));
		long tokenGeneratedTime = Long.parseLong(tokenObj.spApiTokenMap.get("sp_tokenGeneratedTime_" + enterpriseCode));
		...
		logger.endTimer("class: SPApiTokenGenerationUtil | method: isTokenValid -- Ends");
		return true;
	}
	 ...
	 */
	private static SPApiToken fetchNewTokenFromAmazon(String enterpriseCode) {
		logger.beginTimer("class: SPApiTokenGenerationUtil | method: fetchNewTokenFromAmazon -- Starts");
		if (isDebugEnabled) {
		...
		try {
			...
			String sAuthUrl = spApiPropertiesMap.get(AmzCommonConstants.SP_TOKEN_URL);
			String sClientId = spApiPropertiesMap.get(AmzCommonConstants.SP_CLIENT_ID);
			String sClientSecret = spApiPropertiesMap.get(AmzCommonConstants.SP_CLIENT_SECRET);
			String sGrantType = spApiPropertiesMap.get(AmzCommonConstants.SP_GRANT_TYPE);
			String sTimeOut = spApiPropertiesMap.get(AmzCommonConstants.AMZ_TIME_OUT);
			String sRefreshtoken = spApiPropertiesMap.get(AmzCommonConstants.SP_REFRESH_TOKEN);
			...
			postObbj.addParameter(new NameValuePair("refresh_token", sRefreshtoken));
			...
			if (code == HttpStatus.SC_OK) {
				...
				if (tokenObj == null) {
					tokenObj = new SPApiToken();
				}
				
				Date dt = new Date();
				long sec = dt.getTime() / 1000;
				
				tokenObj.spApiTokenMap.put("sp_token_" + enterpriseCode, (String) jsonResp.get("access_token"));
				tokenObj.spApiTokenMap.put("sp_validity_" + enterpriseCode, String.valueOf(jsonResp.get("expires_in")));
				tokenObj.spApiTokenMap.put("sp_tokenGeneratedTime_" + enterpriseCode, Long.toString(sec));
			} else {
		...
		logger.endTimer("class: SPApiTokenGenerationUtil | method: fetchNewTokenFromAmazon -- Ends");
		return tokenObj;
	...
	private static TrustManager[] trustAllCertificates() {
		logger.info("class: SPApiTokenGenerationUtil | method: trustAllCertificates -- Starts");
		...
		logger.info("class: SPApiTokenGenerationUtil | method: trustAllCertificates -- Ends");
